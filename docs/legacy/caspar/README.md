...
# Archived Caspar documentation

> **Historical, not authoritative.** These pages preserve the behavior and
> terminology of the legacy Caspar interfaces during their compatibility
> window. Paths, commands, architecture, and defaults may no longer describe
> the current Aseman system. Start at the [current documentation](../../README.md)
> and use this archive only when operating a named Caspar compatibility edge.
...
- **use the Caspar client CLI** (`apps/aseman-client/`) to do anything the node's shell
  API exposes,
...
3. **VM plugin** — a standalone Rust project under `modules/runtime/` that implements one
