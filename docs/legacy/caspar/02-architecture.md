...
The entrypoint (`apps/aseman-node/src/main.rs`) wires adapters/tools, installs action
...
- `node/` — main node runtime (binaries `caspar-node`, `caspar-keygen`)
  - `apps/aseman-node/src/` — action router, core transactions, chain, VM manager, telemetry
