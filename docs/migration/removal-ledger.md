...
| RL-015 | **Canonical ownership consolidated, command set incomplete.** `apps/asemanctl` owns both the implementation and its warning alias binary; `apps/aseman-client` owns the compatibility client, and deployable SDK samples live in `examples/creatures`. The proxy roots are deleted. Required administration groups, generated public clients/SDKs, stable structured output, and compatibility-expiry observation remain open. |
