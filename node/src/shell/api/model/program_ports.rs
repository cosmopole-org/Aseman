...
use aseman_domain::program::{ProgramAlarm, ProgramRecord, VmResourceStore, DEFAULT_ALARM_ENTITY};
use aseman_ports::{
...
macro_rules! route {
    ($self:ident, |$ports:ident| $call:expr) => {
        match crate::shell::api::model::core_storage::current_unit() {
            Some(unit) => {
                let $ports = aseman_capsule_repositories::program::CapsuleProgramPorts {
                    repository: &*unit,
                };
                $call
            }
