...
//! (so a container can never spoof another creature's namespace) and forwards it
//! to the backend's host, which makes the call to the node as the container's
//! workload. The result is returned verbatim as the `RESPONSE` payload.

use crate::docker_host::connection::ContainerIdentity;
use crate::docker_host::prelude::*;
use aseman_contracts::guest_api::{
    PROGRAM_TARGET_OPS, TARGET_PROGRAM_ID_KEY, TARGET_VM_ID_KEY, VM_TARGET_OPS,
};
...
pub(crate) fn dispatch_host_call(
    ...
    match identity_packet(&op, input, identity) {
        Some(packet) => host.unified_host_call(&packet).into_bytes(),
