...
fn live_creature_ports_pass_conformance_on_capsules() {
    ...
    assert_eq!(
        ...
        Err(aseman_capsule::CapsuleStoreError::Conflict)
    );
    assert_eq!(
    ...
    let programs = aseman_capsule::program::CapsuleProgramPorts {
        repository: &repository,
    };
    ...
    let stores = aseman_capsule::store::CapsuleStorePorts {
        repository: &repository,
    ...
    aseman_ports::conformance::gateway_routes(
        &aseman_capsule::gateway::CapsuleGatewayRoutes {
            repository: &repository,
        },
        "1@conformance",
        "alice",
    ...
    let entities = aseman_capsule::entity::CapsuleEntityPorts {
        repository: &repository,
    };
