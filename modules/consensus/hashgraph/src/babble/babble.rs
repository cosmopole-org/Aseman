...
use crate::peers::{JSONPeerSet, PeerSet};
...
pub fn load_key_for_config(config: &mut Config) -> Result<()> {
    ...
    if !std::path::Path::new(&keyfile_path).exists() {
        ...
        let new_key = crate::crypto::keys::generate_ecdsa_key()
        ...
        // Mirror the key pair to BABBLE_DATA_DIR so the node's shard bootstrap can copy
...
mod tests {
    use super::*;
    use crate::crypto::keys::{dump_private_key, generate_ecdsa_key, parse_private_key};
